...
def pp_calc(aim, speed, diff, map_speed, num_circles, num_sliders, num_spinners, max_combo, num_objects, misses, c100, c50, used_mods = mods() ,combo = 0xFFFF, score_version = 1, c300 = 0xFFFF):
	res = pp_calc_result()
	od = diff["OD"]
	ar = diff["AR"]
	circles = num_circles
	...
	if c300 == 0xFFFF:
		c300 = num_objects - c100 - c50 - misses
	...
	combo_break = math.pow(combo, 0.8) / math.pow(max_combo, 0.8)
...
def pp_calc_acc(aim, speed, diff, map_speed, num_circles, num_sliders, num_spinners, max_combo, num_objects, acc_percent, used_mods=mods(), combo=0xFFFF, misses=0, score_version=1):
	misses = min(num_objects,misses)
	max300 = (num_objects - misses)
	acc_percent = max(0.0, min(acc_calc(max300, 0, 0, misses) * 100.0, acc_percent))
	c50 = 0
	c100 = round(-3.0 * ((acc_percent * 0.01 - 1.0) * num_objects + misses) * 0.5)

	if c100 > num_objects - misses:
		c100 = 0
		c50 = round(-6.0 * ((acc_percent * 0.01 - 1.0) * num_objects + misses) * 0.2)
		c50 = min(max300, c50)
	else:
		c100 = min(max300, c100)

	c300 = num_objects - c100 - c50 - misses
	return pp_calc(aim, speed, diff, map_speed, num_circles, num_sliders, num_spinners, max_combo, num_objects, misses, c100, c50, used_mods, combo, score_version, c300)
