...
class d_obj:
	def __init__(self, base_object, radius, prev, map_speed):
		...
		if(prev != None):
			self.delta_time = (int(self.ho.time) - int(prev.ho.time)) / map_speed
			if(self.ho.h_type !=  3):
				# Calculate speed
				self.strains[0] = prev.strains[0]*math.pow(consts.decay_base[0],self.delta_time / 1000.0) + self.calculate_speed()*consts.weight_scaling[0]#sp.(prev)
				# Calculate aim
...
def calculate_difficulty(type, objects, map_objects, map_speed):
	strain_step = 400 * map_speed
	prev = None
	max_strain = 0
	decay_weight = 0.9
	highest_strains = []
	interval_end = math.ceil(float(map_objects[0].time) / strain_step) * strain_step
	for obj in objects:
...
def calc_stars(map_objects, diff, map_speed, num_circles, num_sliders, num_spinners, max_combo, num_objects):
    # map = file
    true_objects = []
    radius = (512 / 16) * (1. - 0.7 * (diff["CS"] - 5) / 5)

    star_scaling_factor = 0.0675
    extreme_scaling_factor = 0.5
    prev = None
    for obj in map_objects:
        new = d_obj(obj, radius, prev, map_speed)
        true_objects.append(new)
        prev = new
    aim = calculate_difficulty(1, true_objects, map_objects, map_speed)
    speed = calculate_difficulty(0, true_objects, map_objects, map_speed)
    ...
    return [aim, speed, stars, map_objects, diff, map_speed, num_circles, num_sliders, num_spinners, max_combo, num_objects]
