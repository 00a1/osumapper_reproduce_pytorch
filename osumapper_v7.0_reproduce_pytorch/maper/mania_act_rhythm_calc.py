...
def mania_step5_set_params(note_density=0.24, hold_favor=0, divisor_favor=[0] * divisor, hold_max_ticks=8, hold_min_return=1, rotate_mode=4):
    return note_density, hold_favor, divisor_favor, hold_max_ticks, hold_min_return, rotate_mode;

def mania_step5_load_model(model_file="saved_rhythm_model"):
...
def mania_step5_load_npz():
    fn = "mapthis.npz";

    return read_npz(fn);

def mania_step5_predict_notes(model, npz, params):
...
def mania_step5_build_pattern(rhythm_data, params, pattern_dataset = "mania_pattern_dataset.npz"):
...
def mania_merge_objects_each_key(objs_each_key):
    """
