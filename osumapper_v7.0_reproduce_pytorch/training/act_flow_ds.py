...
def step3_read_maps_pattern(params):
    pattern_length = -1
    result_avail_note_begin = [[] for _ in range(18)]
    result_avail_note_end = [[] for _ in range(18)]
    result_avail_hold = [[] for _ in range(18)]
    result_pattern_note_begin = [[] for _ in range(18)]
    result_pattern_note_end = [[] for _ in range(18)]
    for file in os.listdir(root):
        if file.endswith(".npz"):
